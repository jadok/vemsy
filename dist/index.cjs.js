...
const pathAppConfigs=path.join(process.cwd(),"app","config");class config extends middlewareSetup.Task{async execute(){try{const a=includesAll({dirname:pathAppConfigs,filter:/(.+)\.js$/,excludeDirs:/^\.(git|svn)$/});global.__app.configs=merge({files,logs},a),global.__app.configs.name=global.__app.configs.name||require(path.join(process.cwd(),"package.json")).name;}catch(a){throw new Error(a)}}}

class express$1 extends middlewareSetup.Task{async execute(){__app.server=express();}}

class expressMiddleware extends middlewareSetup.Task{constructor(a){super(a),this.middlewares=a;}async execute(){if("undefined"==typeof __app.server)throw new Error("Server not up.");this.middlewares.forEach(a=>{t(a).isExpressMiddleware?__app.server.use(a):__app.server.use(a());});}}
...
typy.addCustomTypes({isExpressMiddleware:middlewareTypeCheck,isPage:pageTypeCheck,isTemplate:templateTypeCheck});

const type={Page,Template};

class globalLibrary extends middlewareSetup.Task{async execute(){global.t=typy.t;}}

class lift extends middlewareSetup.Task{async execute(){// Set process title
// Bind server to port
...
const filePathToPath=a=>path.join(process.cwd(),...a.split("/"));

class themeInit extends middlewareSetup.Task{async execute(){global.__app.theme={motor:__app.configs.files.theme_motor,name:__app.configs.files.theme_name};const a=filePathToPath(__app.configs.files.app_path.themes+"/"+__app.configs.files.theme_name+"/"+__app.configs.files.app_path.views);__app.server.set("twig options",{allow_async:!0,// Allow asynchronous compiling
strict_variables:!1}),__app.server.set("views",a),__app.server.set("view engine",__app.theme.motor);}}

const includesAll$1=require("include-all");class themePages extends middlewareSetup.Task{async execute(){const a=path.join(__app.configs.files.app_path.themes+"/"+__app.configs.files.theme_name),b={...global.__app},c=includesAll$1({dirname:a,filter:/(.+)\.js$/,excludeDirs:/^\.(git|svn)$/});global.__app=b,global.__app.theme.pages=Object.keys(c.pages).map(a=>(c.pages[a].name=a,c.pages[a]));}}

class logger extends middlewareSetup.Task{async execute(){const a=winston.createLogger({transports:[new winston.transports.File(__app.configs.logs.winston.file),new winston.transports.Console(__app.configs.logs.winston.console)],exitOnError:!1});a.stream={write:b=>a.info(b)},__app.server.use(morgan("combined",{stream:a.stream}));}}
...
var index = {middleware,runner: middlewareSetup.runner,setup,type,Task: middlewareSetup.Task};
